...
The `case`, `in`, and `not in` are somewhat special SODA functions because they take a varying amount of arguments, but also allow the arguments to be a mixture of values, columns, and expressions. With the concepts that we've covered so far, the SodaFunc constructor for these three functions should not be possible, because using a varying list as input to these constructors would restrict all of the inputs to be of the same "kind" of SODA element (i.e. the same "outer type" of the SODA element). To get past this restriction we use `Expr` which we can use to "hide" the "outer type" (we can do this because it's an existential type, but you don't need to understand that to use it). This means that anything in the argument lists of `In`, `NotIn`, and part of the tuples in `In` have to be "hidden" with the `Expr` type using the similarly named `Expr` data constructor. 
